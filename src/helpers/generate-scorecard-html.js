...
export default (content, clientName) => {
    ...
    for (const item of content) {
        if (
            [...neturalStatus, ...failedStatus, ...passedStatus].includes(
                item?.current_status
            )
        ) {
            let color = 'grey'
           ...
           <div>
                ...
                <p style="font-size: 16px;">${item?.summary ?? 'N/A'}</p>
    ...
    let html = `
        <div>
            ...
            <p style="font-size: 16px;">As we closety monitored various bills and their implications, we present this scorecard to highlight the key legislative changes and their impact on our company.</p>
